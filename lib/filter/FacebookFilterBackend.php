...
class FacebookFilterBackend extends sfFilter
{
	...
	public function execute($filterChain){  
		...
		if($user->isAuthenticated()) {
            if(is_null($facebook->getToken()) || is_null($facebook->getSignedRequest())) {
                if($facebook->checkPermissions("email,rsvp_event,user_birthday,user_location,user_likes,manage_pages")) {
