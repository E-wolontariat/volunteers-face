 ...
 *
 * @method PageQuery leftJoinUserEvent($relationAlias = null) Adds a LEFT JOIN clause to the query using the UserEvent relation
 * @method PageQuery rightJoinUserEvent($relationAlias = null) Adds a RIGHT JOIN clause to the query using the UserEvent relation
 * @method PageQuery innerJoinUserEvent($relationAlias = null) Adds a INNER JOIN clause to the query using the UserEvent relation
 *
...
abstract class BasePageQuery extends ModelCriteria
{
    ...
    {
        $sql = 'SELECT `ID`, `FACEBOOK_ID`, `USER_ID`, `ACCESS_TOKEN`, `LAST_SYNC`, `UPDATED_AT`, `CREATED_AT` FROM `page` WHERE `ID` = :p0';
        try {
    ...
    public function filterByUserEvent($userEvent, $comparison = null)
    {
        if ($userEvent instanceof UserEvent) {
            return $this
                ->addUsingAlias(PagePeer::ID, $userEvent->getPageId(), $comparison);
